...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const evoInput = document.getElementById('CurrentEvo');

    creditsInput.addEventListener('input', () => formatInputWithCommas(creditsInput));
    goldInput.addEventListener('input', () => formatInputWithCommas(goldInput));
    evoInput.addEventListener('input', () => formatInputWithCommas(evoInput));
});
...
function doCalculate() {
    ...
    let userCreds = parseInt(document.getElementById('Credits').value.replace(/,/g,'')) || 0;
    let userGold = parseInt(document.getElementById('Gold').value.replace(/,/g,'')) || 0;
    let userEVO = parseInt(document.getElementById('CurrentEvo').value.replace(/,/g,'')) || 0;
    ...
    if (!userEVO || (userCreds <= 0 && userGold <= 0)) {
        document.getElementById('resultEvo').innerHTML =
            `<h1 class="resultText">INGRESA EVO Y CREDITO O ORO</h1>`;
        return;
    }

    // --- CALCULO CON CREDITOS ---
    if(userEVO >= 328){
        ...
        while(true){
            ...
            let cost = Math.floor(creditsData[userEVO]*((100-Discount)*0.01));
            if(userCreds >= cost){
                userCreds -= cost;
                userEVO += 1;
            } else {
    ...
    if(result >= 30){
        ...
        while(true){
            ...
            let cost = Math.round(goldData[result+increase]*((100-Discount)*0.01));
            if(userGold >= cost){
                userGold -= cost;
                increase += 1;
            } else break;
    ...
    // --- MOSTRAR RESULTADO ---
    if(result - startingEVO === 0){
        document.getElementById('resultEvo').innerHTML =
            `<h1 class="resultText">No subiste ningun EVO</h1>
             <h1 class="resultText">No tienes suficiente crédito u oro</h1>`;
    } else {
        document.getElementById('resultEvo').innerHTML =
            `<h1 class="resultText">NUEVO EVO: ${result.toLocaleString('en-US')}</h1>
             <h1 class="resultText">AUMENTASTE: +${(result - startingEVO).toLocaleString('en-US')}</h1>`;
    }
}
